...
func test_dqn_cartpole():
	print("=== CartPole DQN Regression Test ===")
	...
	for ep in range(EPISODES):
		...
		if (ep + 1) % WINDOW == 0:
			...
			print("Episode %d | Avg(%d)=%.2f | ε=%.3f"
				% [ep + 1, WINDOW, avg, policy.epsilon])

	var final_avg = _mean(rewards.slice(-WINDOW))
	print("Final rolling avg:", final_avg)
	
	assert_true(final_avg > SUCCESS_THRESHOLD)
